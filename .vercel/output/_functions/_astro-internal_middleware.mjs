...
import './chunks/astro-designed-error-pages_BscfIlKG.mjs';
import 'piccolore';
import './chunks/astro/server_SWj2Xmpp.mjs';
import 'clsx';
import { s as sequence } from './chunks/index_nFuPHG1p.mjs';
...
async function getActiveRedirects() {
  ...
  redirectCache = rows;
  redirectCacheExpiresAt = now + 6e4;
  return rows;
}
function isSameOriginOrLoopback(originValue, expectedValue) {
  try {
...
const onRequest$1 = async (context, next) => {
  ...
  if (method === "GET" && !context.url.pathname.startsWith("/api/")) {
    const redirects = await getActiveRedirects();
    const currentPath = context.url.pathname;
    const match = redirects.find((item) => item.fromPath === currentPath);
    if (match) {
      return context.redirect(match.toPath, match.statusCode);
    }
  }
  if (isWrite && context.url.pathname.startsWith("/api/") && context.url.pathname !== "/api/admin/login") {
  ...
  if (isWrite && context.url.pathname.startsWith("/api/admin/") && context.url.pathname !== "/api/admin/login") {
    if (isLocalDevRequest(context.url) && process.env.ADMIN_LOCAL_BYPASS !== "false") {
      ...
      response2.headers.set("Permissions-Policy", "camera=(), microphone=(), geolocation=(self)");
      response2.headers.set("Strict-Transport-Security", "max-age=63072000; includeSubDomains; preload");
